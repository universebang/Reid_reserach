...
def extract_match(data_match,lock,average_match,match_feature,count_match,register_out):
	...
	print("wen jian list",len(candidate_path_ids))
	for mkey in matching_out:
		try:
			...
			print(mobject_id)
			print(mcamera_id)
			print("last result",len(register_out))
			for rkey in register_out.keys():
				...
				if robject_id==mobject_id:
					print("find-------------")
					print(mobject_id-1)
					print(len(candidate_path_ids))
					print("--------------",candidate_path_ids[mobject_id-1])
...
def main():
	...
	for count in range(5):
		print type(multiprocessing_register[i])
		...
		p = multiprocessing.Process(target=extract_register,args=(multiprocessing_register[count],lock,average_register,register_feature,count_register))
		p.start()
	...
	for j in range(5):
		...
		p = multiprocessing.Process(target=extract_match,args=(multiprocessing_match[j],lock,average_match,match_feature,count_match,register_out))
		p.start()
