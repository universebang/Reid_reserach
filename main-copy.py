...
candidate_path = '../candidate_2018_09_14_5/'
cam2_dict = {}
cam1_image_list = []
cam2_image_list = []
...
def extract_register(data_register,lock,count,length,register_feature): #,average_register,register_feature,count_register):
...
def extract_match(data_match,lock,j,length, register_out): #,match_feature): #,average_match,count_match,register_out):
	#lock.acquire()
	#global count_match
	#lock.acquire()
	...
	mm.free()
	#lock.release()
# def test_distance(matching_out,register_out):
# 	candidate_path_ids = []
# 	for i in range(0, len(cam1_image_list)):  # len(cam1_image_list)):
# 		# print(i)
# 		id = cam1_image_list[i].split('/')[-2]
# 		candidate_path_id = candidate_path + id
# 		if not os.path.exists(str(candidate_path_id)):
...
def main():
	...
	for count in range(5):
		...
		p = multiprocessing.Process(target=extract_register,args=(multiprocessing_register[count],lock,count,length,register_feature)) #,average_register,register_feature,count_register))
		p.start()
	...
	# for i in range(len(register_feature)):
	# 	print(i,register_feature[i])
	...
	for j in range(5):
		...
		p = multiprocessing.Process(target=extract_match,args=(multiprocessing_match[j],lock,j,length,register_out)) #,match_feature)) #,average_match,count_match,register_out))
		p.start()
